      ...
      body: SafeArea(
        child: ListView(padding: const EdgeInsets.only(bottom: 24), children: [
          const Padding(
            ...
            child: Text('Compare', style: TextStyle(color: AppColors.ink, fontSize: 24, fontWeight: FontWeight.bold)),
          ),
          const Padding(
            padding: EdgeInsets.fromLTRB(20, 0, 20, 8),
            child: Text('Overlay rate history · pick up to 4', style: TextStyle(color: AppColors.faint, fontSize: 13)),
          ),
          ...
          SizedBox(
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, i) {
                ...
                final active = idx >= 0;
                final color = active ? _lineColors[idx % _lineColors.length] : AppColors.line;
                return GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: active ? color.withOpacity(0.12) : AppColors.panel,
                      borderRadius: BorderRadius.circular(999),
                      border: Border.all(color: color),
                    ),
                    child: Text(f.name.split(' ').first,
                        style: TextStyle(color: active ? color : AppColors.mute, fontSize: 13)),
                  ),
                );
              },
            ),
          ...
          _Card(child: SizedBox(height: 190, child: _Overlay(selected: _selected, histories: histories))),
          const Padding(
            padding: EdgeInsets.only(top: 2),
            child: Center(child: Text('Gross effective annual yield · last 12 months', style: TextStyle(color: AppColors.faint, fontSize: 11))),
          ),

          if (_selected.isNotEmpty) ...[
            const Padding(
              padding: EdgeInsets.fromLTRB(22, 16, 22, 4),
              child: Text('NET YIELD AFTER 15% TAX',
                  style: TextStyle(color: AppColors.faint, fontSize: 12, letterSpacing: .5, fontWeight: FontWeight.w600)),
            ),
            ..._selected.asMap().entries.map((e) {
              ...
              return Padding(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Text(f?.name ?? e.value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: AppColors.ink, fontSize: 14))),
                  if (gross != null) ...[
                    Text('${gross.toStringAsFixed(2)}%', style: const TextStyle(color: AppColors.faint, fontSize: 13)),
                    const SizedBox(width: 12),
                    Text('${(gross * 0.85).toStringAsFixed(2)}%', style: const TextStyle(color: AppColors.gold, fontSize: 15, fontWeight: FontWeight.w600)),
                  ] else
            ...
            const Padding(
              ...
              child: Text('Tap a fund to add or remove it. Net strips the 15% withholding tax.',
                  style: TextStyle(color: AppColors.faint, fontSize: 11, height: 1.5)),
            ),
          ] else
            const Padding(
              padding: EdgeInsets.all(30),
              child: Center(child: Text('Pick funds above to compare their yields.', style: TextStyle(color: AppColors.mute))),
            ),
        ]),
      ),
    );
...
class _Overlay extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (perFund.isEmpty) {
      return const Center(
        child: Text('History builds as rates update.\nCompare net yields below.',
            textAlign: TextAlign.center, style: TextStyle(color: AppColors.mute, fontSize: 13)),
      );
    }
    ...
    perFund.forEach((id, pts) {
      ...
      bars.add(LineChartBarData(spots: spots, isCurved: true, color: color, barWidth: 2, dotData: const FlDotData(show: false)));
    });
    ...
    return LineChart(LineChartData(
      ...
      maxY: hi! + pad,
      gridData: FlGridData(show: true, drawVerticalLine: false, getDrawingHorizontalLine: (_) => const FlLine(color: AppColors.line, strokeWidth: 1)),
      ...
      titlesData: FlTitlesData(
        ...
        leftTitles: AxisTitles(sideTitles: SideTitles(
          ...
          getTitlesWidget: (v, _) => Text('${v.toStringAsFixed(0)}%', style: const TextStyle(color: AppColors.faint, fontSize: 10)),
        )),
      ),
