...
class AppTheme {
  ...
  static const initial =
      AppTheme(mode: ThemeMode.system, accent: AkibaAccent.gold, textScale: 1.0);

  AppTheme copyWith({ThemeMode? mode, AkibaAccent? accent, double? textScale}) =>
      AppTheme(
        mode: mode ?? this.mode,
...
const _kAccent = 'theme_accent'; // stored as AkibaAccent.index
...
class ThemeController extends Notifier<AppTheme> {
  ...
  void setAccent(AkibaAccent accent) {
  ...
  static AkibaAccent _accentFrom(int? i) =>
      (i != null && i >= 0 && i < AkibaAccent.values.length)
          ? AkibaAccent.values[i]
...
final themeModeProvider =
    Provider<ThemeMode>((ref) => ref.watch(themeControllerProvider).mode);
final accentProvider =
    Provider<AkibaAccent>((ref) => ref.watch(themeControllerProvider).accent);
final textScaleProvider =
    Provider<double>((ref) => ref.watch(themeControllerProvider).textScale);
